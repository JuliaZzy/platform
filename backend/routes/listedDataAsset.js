...
router.post('/query', (req, res) => {
  ...
  const countSql = `
    ...
    FROM non_listed_companies_20250414
    ${whereClause}
  `;
...
router.get('/options', (req, res) => {
  ...
  const queries = fields.map(field =>
    new Promise((resolve, reject) => {
      db.query(
        `SELECT DISTINCT \`${field}\` AS value FROM non_listed_companies_20250414 ORDER BY \`${field}\``,
        // `SELECT DISTINCT \`${field}\` AS value FROM non_listed_companies_20250414 WHERE hide_flag != "是" ORDER BY \`${field}\``,
        (err, results) => {
          if (err) reject(err);
...
router.get('/search/company', (req, res) => {
  ...
  db.query(
    `SELECT DISTINCT company_name FROM non_listed_companies_20250414
     WHERE company_name LIKE ? LIMIT 20`,
...
router.get('/search/content', (req, res) => {
  ...
  if (!query) return res.json([]);
  db.query(
    `SELECT DISTINCT dataasset_content FROM non_listed_companies_20250414
     WHERE dataasset_content LIKE ? LIMIT 20`,
